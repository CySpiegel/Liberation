...
"_ls","_unitdir","_arr","_coverobj","_nearestcoppos"];
 ...
 if (_unit getvariable "doomed" == 0) then 
 {                //This will check if this unit got already spawn function. Cause we dont want to fire it twice or more...
...
while {true} do
{
    ...
    _dragger = selectRandom (_nearestunits - (playableUnits + switchableUnits + [_unit]));
    sleep 1;
    if ((alive _unit) && ((side _dragger) != civilian) 
    && ((lifeState _dragger) != "INCAPACITATED") 
    && (alive _dragger) 
    && ((AnimationState _dragger) != "AcinPknlMwlkSrasWrflDb")) exitWith {};
...
if ((alive _unit) && (side _dragger != civilian) 
&& (!isPlayer _dragger) && (_dragger != _unit) 
&& (_hls != "INCAPACITATED") && (alive _dragger) 
&& ((AnimationState _dragger) != "AcinPknlMwlkSrasWrflDb")) then 
{
